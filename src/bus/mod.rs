...
impl Bus {
    ...
    pub fn read(&self, addr: usize) -> Result<u32, BusError> {
            ...
            && addr < self.memory_layout.rom_start + self.memory_layout.rom_size
        {
            Ok(self
                .rom
                .read(Self::get_offset(addr, self.memory_layout.rom_start)))
        } else if addr >= self.memory_layout.ram_start
            && addr < self.memory_layout.ram_start + self.memory_layout.ram_size
        {
            Ok(self
                .ram
                .read(Self::get_offset(addr, self.memory_layout.ram_start)))
        } else {
            Ok(0_u32)
        }
    }
    ...
    pub fn write(&self, addr: usize, val: u32) -> Result<(), BusError> {
        ...
        {
            self.rom
                .write(Self::get_offset(addr, self.memory_layout.rom_start), val);
        } else if addr >= self.memory_layout.ram_start
            && addr < self.memory_layout.ram_start + self.memory_layout.ram_size
        {
            self.ram
                .write(Self::get_offset(addr, self.memory_layout.ram_start), val);
        }
...
fn test() {
    fn test_read(bus: &Bus, addr: usize) {
        let val = bus.read(addr).expect("read error");
